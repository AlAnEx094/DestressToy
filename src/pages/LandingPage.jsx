...
const galleryItems = [
  ...
  { label: 'Плюш · корпоративные подарки', title: 'Плюшевый медведь', body: 'Классический мягкий медведь с фирменной вышивкой логотипа. Подходит для подарков сотрудникам, партнёрам и клиентам.', image: '/images/gallery/plush_bear.webp', alt: 'Плюшевый медведь с вышивкой логотипа — кастомный мягкий маскот для корпоративных подарков' },
...
const useCases = [
  ...
  },
  {
    ...
    body: 'Мягкий бренд-объект для команды, внутренних запусков и корпоративных дат. Его не убирают сразу после вручения.',
  },
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
const reviews = [
  ...
  {
    name: 'Ирина Н.',
    role: 'Менеджер по маркетингу, производственная компания',
    city: 'Екатеринбург',
  ...
  {
    name: 'Екатерина М.',
    role: 'Event-менеджер, логистическая компания',
    city: 'Краснодар',
    text: 'Игрушки получились хорошие, но доставка задержалась на несколько дней, из-за этого понервничали перед мероприятием. В итоге всё пришло целым, а на следующий тираж нам сделали скидку.',
    qty: '300 шт',
    type: 'Event-раздатка',
    rating: '★★★★☆',
  },
    ...
    <main className="bg-[#151716] text-[#151716]">
      ...
      <section id="pricing" className="bg-[#151716] py-10 md:py-16 xl:py-24">
        <Container>
          ...
          {pricingTab === 'plush' && (
            <p className="mt-4 text-sm text-[#7c847d]">Классическая мягкая игрушка. Плюшевый ворс. Подходит для корпоративных подарков и промо-наборов.</p>
          )}
        ...
        <Container>
          <div className="grid items-center gap-8 md:gap-10 md:grid-cols-[minmax(0,0.95fr)_minmax(0,1.15fr)]">
            <div className="order-2 md:order-1 max-w-[520px]">
              ...
              <h2 className="text-[1.75rem] md:text-[2.5rem] xl:text-[3rem] font-bold leading-[1.1] tracking-[-0.02em] text-[#151716]">
                Бренд-игрушка работает дольше обычной раздатки
              </h2>
              <p className="mt-6 text-lg leading-8 text-[#5a6060]">
                Её берут в руки, ставят на стол, дарят коллегам или клиентам. Такой предмет не конкурирует с листовками,
                ручками и блокнотами — он выглядит как отдельный подарок от бренда.
              </p>

              <div className="mt-8 flex flex-wrap gap-3">
                {['Подарки сотрудникам', 'Промонаборы', 'Event-пакеты', 'Подарки партнёрам'].map(
                  (tag) => (
                    <span
                      key={tag}
